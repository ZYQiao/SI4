...
%type   <ast>   stmt expr var_decl type var call defcond
%type   <ast>   func_decl prototype expr_opt init_var catch_opt finally_opt
...
stmt:           ';'                     { $$ = make_expr_statement(NULL); }
        ...
        |       KTRY stmt catch_opt finally_opt
...
cond_list:      cond_list KCASE expr ':' stmt
                                        { list_append($1, $3, FREE_NODE);list_append($1, $5, FREE_NODE);$$ = $1; }
        |       /* empty */             { $$ = list_create(); }
            ;
defcond:        KDEFAULT ':' stmt       { $$ = $3; }
        |       /* empty */             { $$ = NULL; }
            ;
catch_opt:      KCATCH stmt             { $$ = $2; }
        |       /* empty */             { $$ = NULL; }
            ;

finally_opt:    KFINALLY stmt           { $$ = $2; }
        |       /* empty */             { $$ = NULL; }
            ;
...
expr:
        ...
        |       expr PE expr          { $$ = make_expression("+=",  barith, 2, $1,$3);}
        |       expr ME expr          { $$ = make_expression("-=",  barith, 2, $1,$3);}
        |       expr MTE expr         { $$ = make_expression("*=",  barith, 2, $1,$3);}
        |       expr DE expr          { $$ = make_expression("/=",  barith, 2, $1,$3);}
        ;
