    ...
    case 2:
      if (EXPRESSION_KIND(node)==comp &&
        ...
        emit(")");
          ...
          AST_TYPE(EXPRESSION_OP1(node)) == string_type && AST_TYPE(EXPRESSION_OP2(node)) == string_type){
        emit("_toy_concat("); code(EXPRESSION_OP1(node));
        emit(", ");  code(EXPRESSION_OP2(node));
        emit(")");
      } else {
...
void produce_code_switch_statement(ast_node *node){
  ...
  FORLIST(p, n->parameters){
    if(i == 0){
      ...
      emit(" else if("); code(list_item_data(p)); emit(")");
      i++;
      continue;
    }
  ...
  if(i == 0){
    emit("{");  code(n->expr_default); emit("}");
  }
  else{
    emit("else {");
  code(n->expr_default); emit("}");
  }
}
...
void produce_code_try_statement(ast_node *node) {
  ...
  emit("{\n");
  emit("TRY\n"); 
  produce_code(n->try_stmt); 
  emit("CATCH\n");
  if (n->catch_stmt != NULL) {
    produce_code(n->catch_stmt);
  } else {
    emit("{}\n");
  }
  indent(+1);
  emit("FINALLY\n");
  if (n->finally_stmt != NULL) {
    produce_code(n->finally_stmt);
  } else {
    emit("{}\n");
  }
  emit("ENDTRY;\n");
