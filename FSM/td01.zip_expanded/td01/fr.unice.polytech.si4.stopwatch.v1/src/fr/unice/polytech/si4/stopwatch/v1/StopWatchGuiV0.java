package fr.unice.polytech.si4.stopwatch.v1;
...
public class StopWatchGuiV0 extends Application{
	...
	ArrayList<Object> allSubscriptions = new ArrayList<>();
	...
	public void start(Stage stage) throws NoSuchMethodException, SecurityException {
    	...
    	SCInterface.time.subscribe(e -> doTime());
    	SCInterface.date.subscribe(e -> doDate());
    	SCInterface.date.subscribe(e -> doRecord());
...
//				}
		...
		midButton.setOnAction(new EventHandler<ActionEvent>() {
			@Override
			public void handle(ActionEvent event) {
				theFSM.getSCInterface().raiseMidB();
			}
...
//				}
		...
		setupUI(stage, leftButton, midButton,rightButton);
	}
	...
	private void doDate() {
		...
		timeValue.setText(year + "--" + month + "--" + day);
	}
	...
	private void doTime() {
		...
		timeValue.setText(hour + ":" + minute + ":" + second);
	}
...
//    	millis = endTime - startTime;
	...
	private void setupUI(Stage stage, Button leftButton, Button midButton, Button rightButton) {
		Scene scene = null;
		VBox vBox = null;
		HBox hBox = null;
		hBox = new HBox(80);
		hBox.setAlignment(Pos.CENTER);
		hBox.getChildren().addAll(leftButton,midButton,rightButton);
		vBox = new VBox(30);
