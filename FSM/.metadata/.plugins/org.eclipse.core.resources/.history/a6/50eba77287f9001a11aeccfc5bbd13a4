...
«««					}
	...
	def dispatch functionImplementation(Check it) '''
		...
		private boolean «functionName»() {
			« IF it.condition instanceof ElementReferenceExpression && ((it.condition as ElementReferenceExpression).reference.class.name.contains("TimeEvent"))»
			boolean isPresent = «code.toString.trim»;
			«code.toString.trim» = false;
			return isPresent;
			«ELSE»
